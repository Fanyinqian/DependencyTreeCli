...
const processJsonData = async (): Promise<GraphData> => {
  ...
  //构建节点数据
  const nodes: NodeConfig[] = Object.keys(jsonData).map((key: string) => {
    const newNode = {
      id: key,
      label: key,
...
const Sidebar: React.FC<SidebarProps> = ({ onColorChange, onLayoutChange }) => {
  ...
  const chooseFun = (itemId: number) => {
    itemId > 3 ? onLayoutChange(layout[itemId]) : onColorChange(color[itemId])
  }
  ...
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      if (
        ...
        !(containerRef.current as HTMLDivElement).contains(event.target as Node)
      ) {
        setActiveItemId(0)
      }
    }
  ...
  return (
    <div className="sidebar">
      ...
      <div
        className={`container ${activeItem ? 'active' : ''}`}
        ref={containerRef}
      >
        {activeItem && (
            ...
            <div className="tooltip-left">
              {activeItem.subcontents.map((subcontent) => (
                <div
                  ...
                  // 各个内容的id
                  onClick={() => {
                    console.log(`Clicked ${subcontent.id}`)
                    chooseFun(subcontent.id)
                  }}
                >
                  <p>{subcontent.text}</p>
                </div>
    ...
    <>
      ...
      <div
        ...
        style={{
          width: '100%',
          height: '100vh',
          backgroundColor: bgC[chooseColor]
        }}
      ></div>
    </>
  )
